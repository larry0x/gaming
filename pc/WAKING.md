...
## Waking the TV from the PC

The TV is a Hisense 100U7KQ running VIDAA OS, woken over its own network control channel by [`wake-tv.py`](./wake-tv.py), which the resume hook in `configuration.nix` runs on every resume. From that trigger, the chain is:
...
4. If the TV reports standby (`statetype == "fake_sleep_0"`) the script publishes the `KEY_POWER` remote key; if the TV is already on it does nothing, so a running TV is never toggled off.
5. The panel comes on, landing on the PC's input.
...
- **Reserve the TV's address.** Give the TV a DHCP reservation for `192.168.1.74`, so the address hard-coded in `wake-tv.py` stays valid.
...
- A VIDAA firmware update that changes the MQTT auth protocol (the app's "protocol version" thresholds) would break credential generation. The fix is to update the algorithm constants in `wake-tv.py`, or re-pair with an updated `pyvidaa`.
...
```sh
sudo python3 pc/wake-tv.py
```

With the TV in standby it prints `TV asleep, sent KEY_POWER` and the set comes on in a few seconds; run again with the TV on and it prints `TV already on, nothing to do` without toggling it off. End to end: `sudo systemctl suspend` (over SSH) or Steam → Sleep, then wake the PC with a keypress — the TV should power on by itself. The script's output line lands in `journalctl -u post-resume.service`.
...
- **A blind `KEY_POWER` press.** The simplest possible script would just send the key — but `KEY_POWER` is a toggle, so it would switch _off_ a TV that is already on when the PC resumes. The state check in step 4 is why the script is a little longer.
